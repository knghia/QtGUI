...
void MainWindow::setup_gui(void){
    ...
    ui->graphics->xAxis->setLabel("Time (s)");
    ui->graphics->yAxis->setLabel("Temperature");
    ui->graphics->xAxis->setRange(0, 10);
    ui->graphics->yAxis->setRange(0, 100);
    ui->graphics->graph(0)->setPen(QPen(Qt::red));
    ...
    timer_plot->setInterval(100);
    timer_plot->start();

    connect(timer_plot, &QTimer::timeout,this, &MainWindow::show_data_graphics);
}
...
void MainWindow::receive_message(void){
    if(is_connect == true){
        QByteArray dataBA = serial_port.readLine();
        QString data(dataBA);
        if (data.size() == 7){
            if( (data[0] == ':') && (data[1] == 'T') && (data[2] == '=') ){
                temperature = data.mid(3, 2).toInt();
            }
        }
...
void MainWindow::show_data_graphics(void){
    if (is_connect == true){
        static QVector<double> x_axis;
        static QVector<double> y_axis;
        static int limit_axis = 10;

        this->time+=0.01;
        ui->time_la->setText(QString::number(this->time));

        x_axis.append(this->time);
        y_axis.append(this->temperature);

        if(this->time > limit_axis){
            limit_axis += 10;
            ui->graphics->xAxis->setRange(this->time - 2, limit_axis);
        }
        ui->graphics->graph(0)->setData(x_axis, y_axis);
        ui->graphics->replot();
    }
}
